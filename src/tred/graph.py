...
class Raster(nn.Module):
    ...
    '''
    def __init__(self, velocity, grid_spacing, pdims=(1,2), tdim=-1, nsigma=3.0, **kw):
        '''
        ...
        xyz_limit = kw.get("xyz_limit", [1000, 1000, 1000])
        constant(self, 'xyz_limit', xyz_limit, dtype=torch.int32)
    ...
    def forward(self, sigma, time, charge, tail, head=None):
        ...
        rasters, offsets = raster_steps(self.grid_spacing, tail, head, sigma, charge, nsigma=self.nsigma, xyz_limit=self.xyz_limit)
