...
for i in ${event_ids[@]}; do
  ...
  OutFile="/home/yousen/Documents/NDLAr2x2/MuonLArSim/build/pgun_mu_3GeV_2mm_event_id${i}_delay18_tred.npz"
  echo "uv run tred -c config_for_pgun.yaml fullsim -i $InFile -o $OutFile"
  CUDA_LAUNCH_BLOCKING=1 uv run tred -c config_for_pgun_delay18.yaml fullsim -i $InFile -o $OutFile
done
